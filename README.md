...
- **TTS**：豆包火山引擎 / MiniMax / OpenAI TTS / ElevenLabs 多选；分句播放时会广播 `tts:audio_ready` 音频段元数据，并支持 WebSocket 客户端显式订阅 TTS 音频块或直接发送 `tts:speak` 合成请求
