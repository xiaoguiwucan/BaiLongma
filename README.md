...
- `v2.4.0`：本地 KWS/openWakeWord 唤醒大版本：新增文本/混合/纯 KWS 唤醒模式、本地 openWakeWord 运行时、KWS 模型扫描/导入/下载、应用配置和录音自测，让唤醒词检测与 ASR 指令识别分离。
- `v2.3.2`：语音唤醒稳定性补丁版：视频/音乐 pre-roll 预录音门控、语音调试面板显示 VAD/声纹/唤醒/媒体门控细节，并修复 partial ASR 可能绕过唤醒门控触发自动发送的问题。
- `v2.3.1`：Mac Electron 打包部署补丁版：默认构建切到 macOS，新增 `build:mac`/`publish:mac`，保留 Windows 打包命令，修正 GitHub 发布仓库，加入 macOS `.icns` 图标和跨平台清理脚本。
- `v2.3.0`：本地语音稳定性大版本：一键准备、真实自测、服务复用、声纹备份/恢复/阈值校准、隐私诊断包、麦克风自检与触发阈值校准，并把麦克风状态并入本地语音总览。
- `v2.2.0`：小智式语音设备控制台大版本，新增外部语音客户端、事件历史、链路总控、一键自检和设备接入包。
- `v2.1.240`：新增 `GET /voice/events/clients` 客户端诊断端点，集中查看连接身份、订阅状态、capabilities 和 negotiated 音频模式。
- `v2.1.239`：新增 /voice/events audio_negotiation 能力，client:accepted 返回 negotiated.audioMode，按 binary_audio/base64_audio 能力给出音频模式建议。
- `v2.1.238`：增强 `npm run voice:events` 调试客户端，新增 protocol 命令、client:hello 身份/能力参数和 CLI smoke 测试，方便小智式 ESP32/桥接客户端联调。
- `v2.1.237`：client:hello 支持 capabilities 声明，/voice/events/status 增加 lastSeenAt 诊断，便于判断客户端能力和活跃状态。
- `v2.1.236`：新增 /voice/events client:hello/client:identify 外部客户端身份登记，status 返回 clientDetails，便于 ESP32/调试客户端诊断并为后续设备配对打基础。
- `v2.1.235`：为 /voice/events 的 tts:speak 增加远端地址级冷却，防止同一设备多开 WebSocket 绕过单连接限流，并在 protocol metadata 暴露 scopes。
- `v2.1.234`：为 /voice/events WebSocket 增加可选 token 鉴权元数据和 upgrade 访问检查，复用 BAILONGMA_API_TOKEN，提升 LAN/外部设备接入安全性。
- `v2.1.233`：把 /voice/events 的 tts:speak 最大文本长度和单连接冷却改为可配置，并在设置页、/settings/tts、/voice/events/protocol 和 hello 中同步生效。
- `v2.1.232`：为 WebSocket `tts:speak` 增加 800 字最大文本限制和 1200ms 单连接冷却，协议元数据公开限制并返回 `text_too_long` / `rate_limited` 错误。
...
v2.4.0 收束 v2.3.2 之后的 KWS 唤醒改造，目标是解决“视频/别人说话容易误触发”和“只靠 ASR 文本匹配唤醒词不够稳定”的问题。默认仍保持文本唤醒，用户可以在设置里切换到混合唤醒或纯本地 KWS。

新增/调整：

- **唤醒检测方式可切换**：设置页新增“文本唤醒 / 混合唤醒（KWS + 文本）/ 本地 KWS”。
- **openWakeWord 本地运行时**：前端语音链路向本地 SenseVoice 服务发送 `kws_detect`，服务端懒加载 `.onnx` 模型并返回 `kws_result` / `kws_status`。
- **纯 KWS 安全门控**：纯 KWS 模式下，只有本地 KWS 命中后才接受后续 final ASR 指令，避免仅靠视频字幕/他人语音文本误唤醒。
- **模型管理闭环**：设置页支持检测 KWS 状态、安装 openWakeWord 依赖、扫描 `models/kws`、导入本地 `.onnx` 路径、从 URL 下载模型、应用配置和录音自测。
- **调试事件**：新增 `wake:kws` / `bailongma:kws-wake`，可观察命中模型、分数、阈值和拒绝原因。
- **诚实限制**：项目不内置第三方 KWS 权重；`sherpa-onnx` 目前只保留配置入口并提示缺少完整模型组，不假装已可用。

推荐用法：先选择“混合唤醒”，导入 openWakeWord `.onnx` 模型并录音自测；稳定后再考虑切换到“本地 KWS”。

---

## v2.3.2 语音唤醒稳定性补丁版

v2.3.2 收束 v2.3.1 之后的三个语音稳定性检查点，重点解决视频/音乐播放时唤醒词开头被盖住、问题难以诊断、以及 ASR partial 中间结果不应成为正式指令的问题。

新增/调整：

- **视频/音乐 pre-roll 预录音门控**：播放媒体时保留最近 0.8–4 秒麦克风环形缓存；检测到近场人声后先把预录音送入 ASR，减少唤醒词/指令开头丢失。
- **语音调试面板增强**：显示麦克风/VAD 当前值、峰值、阈值，唤醒通过/拒绝原因与置信度，声纹分数/阈值，媒体门控和 pre-roll 状态。
- **partial ASR 安全分离**：ASR partial 只用于显示/调试，不再写入待发送指令，也不会触发自动发送；只有 final 且通过唤醒/声纹门控后才会发送给助手。
- **视频抗干扰预设更新**：预录音缓存纳入设置、后端配置、稳定性预设、readiness/doctor 检查和 smoke 覆盖。

---

## v2.3.1 Mac Electron 打包部署补丁版

v2.3.1 先解决“Mac 上到底怎么打包安装”的问题，把默认构建路径从 Windows NSIS 改为 macOS Electron 包，并修正发布仓库到当前维护仓库。

新增/调整：

- **默认 Mac 构建**：`npm run build` 现在等同于 `npm run build:mac`。
- **Mac 发布命令**：新增 `npm run publish:mac`，`npm run publish` 默认走 Mac 发布。
- **保留 Windows 兼容**：原 Windows 打包保留为 `npm run build:win` 和 `npm run publish:win`。
- **跨平台清理**：新增 `scripts/prebuild-clean.mjs`，Mac 不再依赖 PowerShell 清理 `dist/`。
- **macOS 图标**：由现有卡通图标生成 `build/icon.icns`，Electron Builder 的 Mac 包使用该图标。
- **发布仓库修正**：electron-builder publish 从旧上游 `xiaoyuanda666-ship-it/BaiLongma` 改为当前维护仓库 `xiaoguiwucan/BaiLongma`。
...
构建产物会输出到 `dist/`，通常包含 arm64 `.dmg` 和 `.zip`。如果只是本机开发运行，仍然使用 `npm start`；Intel Mac 另用 `npm run build:mac:x64`。

---

## v2.3.0 本地语音稳定性大版本

v2.3.0 把 v2.2.0 之后连续多个本地语音检查点收束成一次正式 Release，目标是让 Mac 本地语音助手在视频播放、多人说话、声纹门控和麦克风阈值问题下更可诊断、更可恢复。

新增能力：

- **一键语音准备**：自动应用本地 SenseVoiceSmall、严格唤醒和视频抗干扰稳定基线。
- **真实语音自测**：用 wake/asr/tts 事件判断本地语音链路是否真的跑通。
- **服务复用与控制**：检测并复用已有 3723 本地 ASR 服务，避免重复启动；支持停止/取消跟踪和按当前模型重启。
- **声纹安全闭环**：支持声纹状态、清除、离线清除、备份、选择备份恢复、阈值校准和防锁死修复。
- **隐私诊断包**：导出不含 API Key、原始音频、声纹文件内容的本地语音诊断 JSON。
- **麦克风自检与阈值校准**：显示当前/峰值/噪声底/阈值，一键按当前峰值校准触发阈值。
- **本地语音总览**：集中显示服务、readiness、声纹、自测、麦克风状态和下一步动作。
...
v2.2.0 是一次大版本更新，不再按小修小补拆分 Release。它把 v2.1.x 中逐步建立的 `/voice/events` 协议、TTS 分句、客户端身份、能力协商和调试客户端，收敛成一个可在 Brain UI 中直接使用的“语音设备控制台”。

新增能力：

- **外部客户端面板**：在“设置 -> 语音识别 -> 外部语音客户端”中查看 ESP32、手机桥接器、CLI 调试客户端是否连接。
- **协议自检**：显示 WebSocket、clients、history、summary、check、package 等端点和关键 capability。
- **LAN/ESP32 接入向导**：自动生成本机调试命令、局域网设备命令和 `client:hello` 示例。
- **事件历史时间线**：`GET /voice/events/history` 返回最近 wake/asr/tts/interrupt 事件，Brain UI 可按类型过滤。
- **语音链路总控**：`GET /voice/events/summary` 聚合客户端、音频订阅、二进制订阅、最近唤醒/识别/TTS/中断数量，并给出中文排障建议。
- **一键自检**：`GET /voice/events/check` 检查协议、客户端、握手、订阅、binary audio、最近事件和“唤醒→ASR→TTS”闭环。
- **设备接入包**：`GET /voice/events/package` 生成 README、`.env.voice`、`client-hello.json`、`subscribe.json`、Node 示例和 ESP32 伪配置。
...
curl http://127.0.0.1:3721/voice/events/summary
...
Mac 用户优先从当前维护仓库 [Releases](https://github.com/xiaoguiwucan/BaiLongma/releases) 下载最新 `.dmg` 或 `.zip`（如果该版本已经上传安装包）。首次打开如遇到 macOS 安全提示，可在“系统设置 -> 隐私与安全性”中允许打开。

如果 Release 暂时只有源码包，可使用下面“从源码运行”方式启动。Windows 安装包后续仍通过 `npm run build:win` 生成。
...
# 查看当前协议元数据、capabilities、limits、auth
npm run voice:events -- protocol

# 监听 JSON 事件，并订阅 base64 音频块；默认会发送 client:hello
...
# 可选身份参数：--client-id / --device / --app / --client-version / --platform / --capability；如需旧行为可加 --no-identify。
```
...
```bash
npm run build       # 默认在当前 Mac 上打包 macOS 版本
npm run build:mac   # 明确打包 macOS arm64 dmg/zip（Apple Silicon）
npm run build:mac:x64 # 打包 Intel Mac x64 dmg/zip
npm run build:win   # Windows NSIS 安装包，需要 Windows/PowerShell 环境
npm run publish     # 默认发布 macOS 版本到 GitHub Releases
npm run publish:mac # 明确发布 macOS arm64 版本
npm run publish:mac:x64 # 发布 Intel Mac x64 版本
npm run publish:win # 发布 Windows 版本
```
