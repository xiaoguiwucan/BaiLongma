...
try {
  ...
  await fetch(`${API}/voice/events/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'speaker:rejected', at: Date.now(), detail: { score: 0.52, threshold: 0.63, reason: 'speaker verification failed' } }) })
  const speakerCalibration = await fetch(`${API}/voice/local/speaker/calibration?score=0.52&passed=false&windowMs=600000`).then(r => r.json())
  assert(speakerCalibration.ok === true && speakerCalibration.recommendedThreshold <= 0.50 && speakerCalibration.changed === true && speakerCalibration.actions?.some(item => item.id === 'apply_threshold'), 'local speaker calibration recommends lower threshold from failed owner score', JSON.stringify(speakerCalibration))
  const speakerCalibrationApply = await fetch(`${API}/voice/local/speaker/calibration/apply`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ score: 0.52, passed: false, windowMs: 600000 }) }).then(r => r.json())
  assert(speakerCalibrationApply.ok === true && speakerCalibrationApply.voice?.speakerThreshold <= 0.50 && speakerCalibrationApply.record?.action === 'speaker_calibration', 'local speaker calibration can apply safe recommended threshold and record history', JSON.stringify(speakerCalibrationApply))
  ...
  const selfTestStart = await fetch(`${API}/voice/local/self-test/start`, { method: 'POST' }).then(r => r.json())
  assert(selfTestStart.ok === true && selfTestStart.since && selfTestStart.selfTest?.steps?.some(item => item.id === 'local_process') && (selfTestStart.started === null || typeof selfTestStart.started.external === 'boolean'), 'local voice self-test can start timestamped session and report reused service state', JSON.stringify(selfTestStart))
  await fetch(`${API}/voice/events/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'wake:accepted', at: Date.now(), detail: { text: '龙马' } }) })
  await fetch(`${API}/voice/events/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'asr:final', at: Date.now(), detail: { text: '测试一下' } }) })
  await fetch(`${API}/voice/events/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'tts:stop', at: Date.now(), detail: { reason: 'done' } }) })
  ...
  assert(readinessBefore.ok === true && readinessBefore.steps?.some(item => item.id === 'local_provider') && readinessBefore.steps?.some(item => item.id === 'local_process') && readinessBefore.steps?.some(item => item.id === 'wake_kws') && readinessBefore.steps?.some(item => item.id === 'speaker_voiceprint') && readinessBefore.steps?.some(item => item.id === 'speaker_gate_safe') && Array.isArray(readinessBefore.nextActions), 'local voice readiness wizard exposes guided steps and next actions', JSON.stringify(readinessBefore))
  ...
  const readinessApply = await fetch(`${API}/voice/local/readiness/apply`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ preset: 'balanced' }) }).then(r => r.json())
  assert(readinessApply.ok === true && readinessApply.voice?.asrProvider === 'local' && readinessApply.voice?.localAsrModel === 'sensevoice-small' && readinessApply.voice?.wakeWordEnabled === true && readinessApply.voice?.speakerVerificationEnabled === false && readinessApply.started?.status && typeof readinessApply.started?.external === 'boolean', 'local voice readiness wizard applies local SenseVoice baseline and reports service source', JSON.stringify(readinessApply))
  assert(readinessApply.record?.action === 'voice_readiness_wizard' && readinessApply.voice?.voiceLocalDoctorHistory?.some(item => item.action === 'voice_readiness_wizard'), 'local voice readiness wizard records auditable history', JSON.stringify(readinessApply.record))
  const speakerGateFix = await fetch(`${API}/voice/local/doctor/fix`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'disable_speaker_gate' }) }).then(r => r.json())
  ...
  assert(localDoctorBefore.ok === true && localDoctorBefore.checks?.some(item => item.id === 'provider') && localDoctorBefore.checks?.some(item => item.id === 'process') && localDoctorBefore.checks?.some(item => item.id === 'wake_kws') && Array.isArray(localDoctorBefore.nextActions), 'local voice doctor endpoint exposes readiness checks and next actions', JSON.stringify(localDoctorBefore))
