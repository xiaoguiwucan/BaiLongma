...
try {
  ...
  assert(selfTestStart.ok === true && selfTestStart.since && selfTestStart.selfTest?.steps?.some(item => item.id === 'local_process'), 'local voice self-test can start timestamped session', JSON.stringify(selfTestStart))
  ...
  assert(readinessApply.ok === true && readinessApply.voice?.asrProvider === 'local' && readinessApply.voice?.localAsrModel === 'sensevoice-small' && readinessApply.voice?.wakeWordEnabled === true && readinessApply.voice?.speakerVerificationEnabled === false && readinessApply.started?.status, 'local voice readiness wizard applies local SenseVoice baseline and keeps speaker gate safe', JSON.stringify(readinessApply))
