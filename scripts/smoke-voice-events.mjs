...
try {
  ...
  assert(readinessBefore.ok === true && readinessBefore.steps?.some(item => item.id === 'local_provider') && readinessBefore.steps?.some(item => item.id === 'local_process') && readinessBefore.steps?.some(item => item.id === 'speaker_voiceprint') && Array.isArray(readinessBefore.nextActions), 'local voice readiness wizard exposes guided steps and next actions', JSON.stringify(readinessBefore))
  ...
  assert(readinessApply.ok === true && readinessApply.voice?.asrProvider === 'local' && readinessApply.voice?.localAsrModel === 'sensevoice-small' && readinessApply.voice?.wakeWordEnabled === true && readinessApply.started?.status, 'local voice readiness wizard applies local SenseVoice baseline and starts service', JSON.stringify(readinessApply))
