...
## 2026-05-28 v0.4.19 管理员昵称显示
- 用户不应直接操作 Wechaty sender_id；UI 应显示/搜索微信昵称，但授权判断不能改成昵称匹配，否则同名/改名会造成权限漏洞。

## 2026-05-28 v0.4.20 LLM 设置不可操作排查
- LLM 模型接口本身可用，本地 API 验证“设为当前/新增/删除”成功。用户页面不可操作的诱因是 Docker 未启动导致 Honcho 本地服务不可达，设置页群记忆接口持续 fetch failed，并存在异常路径崩溃风险。
- 处理方式：Honcho 失败进入短期降级，返回可读错误，不再影响模型池设置。

## 2026-05-28 v0.4.21 新增群显示不一致排查
- 新群已进入 Wechaty 缓存和成员昵称库，但 selected=false；设置页部分模块只看已选 @ 回复群，部分模块看统计/成员库，导致显示不一致。
- 解决：提供 known groups 聚合接口，前端所有群相关候选统一从“缓存群 + 已识别群 + 已配置群”合并结果渲染，权限/统计仍由各自勾选控制。
