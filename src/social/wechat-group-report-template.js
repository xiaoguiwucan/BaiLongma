...
function rankRows(rows = [], unit = '次', notes = []) {
  ...
  return safe.map((row, i) => `<div class="rank-row ${i === 0 ? 'first' : ''}">
    <i>${i + 1}</i><span>${esc(shortName(row.name))}</span><b>${esc(row.value || 0)}${unit}</b><em>${esc(notes[i] || '继续保持，榜上有名')}</em>
  </div>`).join('')
}
...
function buildData(stats = {}) {
  ...
  return {
    ...
    moments: [
      `${shortName(msg.name)} ${msg.value || 0} 条：今日嘴替，打字机申请工伤`,
      `${shortName(img.name)} ${img.value || 0} 图：相册像开了自来水`,
      `${shortName(link.name)} 链接上榜：情报局开始营业`,
      `${shortName(brag.name)} 装逼高光：低调失败案例`,
      `${shortName(emoji.name)} 表情包控场：不说话也很响`,
      `全群 ${totals.participant_count || 0} 人参与：今天也在认真热闹`,
    ],
...
function commonBlocks(d) {
  ...
  return {
    ...
    boards: `${board('发言榜','💬',b.messages,'条',['稳定发电，嘴替本替','慢工出细活，句句带货','躺着也能上榜','浪花一朵朵','吹过但留下痕迹'])}${board('发图榜','🖼️',b.images,'张',['图像洪峰预警','边聊边投喂','素材不眠夜','可爱弹药库','随手就是一张'])}${board('表情榜','😁',b.emojis,'个',['表情包法师开大','用图说话派','文字表情双修','咸鱼翻身.gif','萌系火力覆盖'])}${board('装逼榜','😎',b.brag,'次',['空气里都是逼格','关键三分装到了','轻轻一秀','可爱也是实力','低频高光'])}`,
    tags: hotTags(d.hot),
